import { getServerSession } from "next-auth";
import { authOptions } from "@/lib/auth";
import { redirect } from "next/navigation";
import { prisma } from "@/lib/prisma";
import NewClientForm from "./ui/NewClientForm";
import DeleteClientButton from "./ui/DeleteClientButton";
import EditClientButton from "./ui/EditClientButton";

export const dynamic = "force-dynamic";

export default async function ClientsPage() {
    const session = await getServerSession(authOptions);
    if (!session?.user?.email) redirect("/auth/login");

    const user = await prisma.user.findUnique({
        where: { email: session.user.email },
        select: { id: true },
    });
    if (!user) redirect("/auth/login");

    const clients = await prisma.client.findMany({
        where: { ownerId: user.id },
        orderBy: { createdAt: "desc" },
        select: { id: true, name: true, description: true, phone: true, createdAt: true },
    });

    return (
        <div className="min-h-screen bg-gradient-to-br from-rose-50 via-indigo-50 to-emerald-50">
        <header className="mx-auto max-w-6xl px-6 pt-8">
            <div className="flex items-center justify-between">
            <div>
                <h1 className="text-2xl md:text-3xl font-semibold">Clientes</h1>
                <p className="text-gray-500 mt-1">Crea y gestiona tus clientes de forma rápida.</p>
            </div>
            <a
                href="/dashboard"
                className="rounded-xl border border-indigo-200 bg-white/70 px-4 py-2 font-medium shadow-sm hover:bg-indigo-50 transition"
            >
                ← Volver al Dashboard
            </a>
            </div>
        </header>

        <main className="mx-auto max-w-6xl px-6 py-8 space-y-6">
            <NewClientForm />

            <div className="rounded-2xl border border-black/5 bg-white/70 backdrop-blur p-4 shadow-sm">
            <div className="flex items-center justify-between mb-3">
                <div className="text-lg font-semibold">Listado</div>
                <div className="text-sm text-gray-500">{clients.length} cliente(s)</div>
            </div>

            <ul className="divide-y">
                {clients.map((c) => (
                <li key={c.id} className="py-3 flex items-center justify-between">
                    <div>
                    <div className="font-medium">{c.name}</div>
                    <div className="text-sm text-gray-500">
                        {c.description ?? "—"}
                        {c.phone ? ` · ${c.phone}` : ""}
                    </div>
                    </div>

                    {/* Acciones: fecha + editar + eliminar */}
                    <div className="flex items-center gap-3">
                    <div className="text-xs text-gray-400">
                        {new Date(c.createdAt).toLocaleDateString()}
                    </div>
                    <EditClientButton client={c} />
                    <DeleteClientButton id={c.id} />
                    </div>
                </li>
                ))}

                {clients.length === 0 && (
                <li className="py-6 text-gray-500 text-sm">
                    Aún no tienes clientes. Crea el primero arriba.
                </li>
                )}
            </ul>
            </div>
        </main>
        </div>
    );
}
